...
class Store {
    ...
    getAll(callback){
        ...
        fs.readdir(this.directory, (err, data)=>{
            ...
            data.forEach((x) =>{
                let itemID = x.slice(0,10);
                console.log('I am x before the get method!', x);
                console.log('I am itemID before the get method!', itemID);
                this.get(itemID, (data, err)=>{
                    if (err) return callback(err);
                    allData.push(data);
                    console.log('I am the data in the getall function', data);
                    receivedDataCount += 1;
                    console.log('I am the allData in the getall function', allData);
                    if (receivedDataCount === expectedDataCount) return callback(allData);
                });
            });
        });
    }
